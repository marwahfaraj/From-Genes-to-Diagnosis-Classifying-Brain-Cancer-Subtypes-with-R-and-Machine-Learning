...
for (dr_path in dr_files) {
  ...
  write.csv(summary_df,
            file      = paste0("output/model/metrics/summary_nb_", method, ".csv"),
            row.names = FALSE)
  ...
  p <- ggplot(summary_df, aes(x = Strategy, y = MeanAcc, fill = Strategy)) +
    ...
    labs(title = paste("Naive Bayes Accuracy (", toupper(method), ")", sep = ""),
         x     = "Strategy",
         y     = "Mean Accuracy") +
    theme_minimal()
  
  ggsave(filename = paste0("output/model/graphs/nb_accuracy_", method, ".png"),
         plot     = p,
         dpi      = 300,
         width    = 8,
         height   = 5)
...
write.csv(combined,
          file      = "output/model/metrics/summary_nb_all_methods.csv",
          row.names = FALSE)
...
p_all <- ggplot(combined, aes(x = Method, y = MeanAcc, fill = Method)) +
  ...
  geom_errorbar(aes(ymin = MeanAcc - SDAcc, ymax = MeanAcc + SDAcc),
                width = 0.2) +
  scale_fill_manual(values = base_colors) +
  labs(title = "Naive Bayes Accuracy Comparison Across DR Methods",
       x     = "DR Method",
       y     = "Mean Accuracy") +
  theme_minimal()

ggsave(filename = "output/model/graphs/nb_accuracy_comparison.png",
       plot     = p_all,
       dpi      = 300,
       width    = 8,
       height   = 5)

cat("Naive Bayes evaluation complete for PCA and t-SNE.\n",
